...
### Номенклатура Component

- Тип (символ) R- резистор, C- емкость, U- микросхема
...
Карточка учета имеет номер и содержит 
...
Словарь содержит дополнительные свойства, которые могут быть отмечены в карточке- тип корпуса микросхемы
