...
- -v детализизированный вывод в консоль
...
Параметры запроса передаются маотодом POST в JSON. Параметры перечислены в proto/rcr.proto 
...
- PropertyType Типы свойств ('A'- тип кооруса, 'V'- номинальное напряжекие, 'A'- максимальный ток, 'P'- рассеиваемая мощность, '%'- точность) 
- Card Карточка учета номенклатуры - микросхема с учетом исполнения(корпуса) и запасы в разных местах хранения
- Package Место хранения. Место хранения может хранить несколько типов компонент
...
Для Linux для glade установите утилиту xmllint для удаления пробелов в XML файлое ресурса.
